...
Endpoint Agent (recommended):
  ...
  medusa-agent install --customer-id ... --api-key ...
  medusa-agent status
...
def cli(ctx: click.Context, verbose: int, quiet: bool) -> None:
    ...
    Protects MCP server connections with a background agent, gateway proxy,
    and real-time policy enforcement. Also includes 559 ad-hoc security
    checks for scanning and CI/CD integration.

    Use 'medusa-agent' for endpoint agent management.
    """
...
def _load_gateway_policy(path: str | None = None) -> GatewayPolicy:
    ...
    return load_gateway_policy(path)
