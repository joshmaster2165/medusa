    ...
    def generate(self, result: ScanResult) -> str:
        ...
        if result.server_scores:
            ...
            for ss in result.server_scores:
                lines.append(
                    f"| {ss.server_name} | {ss.score}/10 | {ss.grade} | {ss.failed} |"
                )
            lines.append("")
        ...
        if failed_findings:
            ...
            for severity in [Severity.CRITICAL, Severity.HIGH, Severity.MEDIUM, Severity.LOW]:
                ...
                for finding in sev_findings:
                    lines.append(
                        f"### [{severity.value.upper()}] {finding.check_id}: "
                        f"{finding.check_title}"
                    )
                    lines.append("")
                    lines.append(
                    ...
                    if finding.owasp_mcp:
                        lines.append(
                            f"- **OWASP MCP:** {', '.join(finding.owasp_mcp)}"
                        )
                    lines.append("")
        else:
            lines.append("## Findings")
        ...
        lines.append("---")
        lines.append(
            f"*Generated by Medusa v{result.medusa_version} "
