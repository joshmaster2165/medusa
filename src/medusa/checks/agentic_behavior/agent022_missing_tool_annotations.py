    ...
    async def execute(self, snapshot: ServerSnapshot) -> list[Finding]:
        ...
        for tool in snapshot.tools:
            ...
            if isinstance(annotations, dict):
                has_hints = any(annotations.get(hint) is not None for hint in _ANNOTATION_HINTS)
        ...
        if not tools_without_annotations:
            findings.append(
                Finding(
                    ...
                    status_extended=("All tools have MCP annotations with safety hints."),
        ...
        other_missing = [t for t in tools_without_annotations if t not in destructive_without]
        if other_missing:
            sample = other_missing[:5]
            suffix = ""
            if len(other_missing) > 5:
                suffix = f" and {len(other_missing) - 5} more"
            findings.append(
                Finding(
                    check_id=meta.check_id,
                    check_title=meta.title,
                    ...
                    resource_name=snapshot.server_name,
                    status_extended=(
                        f"{missing_count}/{total} tools ({pct:.0f}%) lack MCP annotations."
                    ),
                    evidence=(f"Tools without annotations: {', '.join(sample)}{suffix}"),
                    remediation=meta.remediation,
                    owasp_mcp=meta.owasp_mcp,
                )
            )
