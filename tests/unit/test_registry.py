...
class TestDiscoverChecks:
    ...
    def test_discovers_at_least_one_check(self, registry: CheckRegistry) -> None:
        assert registry.check_count > 0, "Registry should discover at least one check"

    def test_discovers_all_expected_tool_poisoning_checks(self, registry: CheckRegistry) -> None:
        expected_ids = {"tp001", "tp002", "tp003", "tp004", "tp005"}
        found_ids = set(registry.check_ids)
        missing = expected_ids - found_ids
        assert not missing, f"Missing tool poisoning check IDs: {missing}"

    def test_discovers_all_expected_input_validation_checks(self, registry: CheckRegistry) -> None:
        expected_ids = {"iv001", "iv002", "iv003", "iv004", "iv005"}
        found_ids = set(registry.check_ids)
        missing = expected_ids - found_ids
        assert not missing, f"Missing input validation check IDs: {missing}"

    def test_discovers_credential_exposure_checks(self, registry: CheckRegistry) -> None:
        expected_ids = {"cred001", "cred002", "cred003"}
        found_ids = set(registry.check_ids)
        missing = expected_ids - found_ids
        assert not missing, f"Missing credential exposure check IDs: {missing}"

    def test_discovers_privilege_scope_checks(self, registry: CheckRegistry) -> None:
        expected_ids = {"priv001", "priv002", "priv003"}
        found_ids = set(registry.check_ids)
        missing = expected_ids - found_ids
        assert not missing, f"Missing privilege scope check IDs: {missing}"

    def test_discovers_authentication_checks(self, registry: CheckRegistry) -> None:
        expected_ids = {"auth001", "auth002", "auth003", "auth004"}
        found_ids = set(registry.check_ids)
        missing = expected_ids - found_ids
        assert not missing, f"Missing authentication check IDs: {missing}"
    ...
    def test_double_discover_does_not_duplicate(self, registry: CheckRegistry) -> None:
        ...
        assert count_after == count_before, "Discovering checks twice should not increase the count"
...
class TestGetChecks:
    ...
    def test_filter_by_category(self, registry: CheckRegistry) -> None:
        ...
        assert len(checks) >= 5, f"Should find at least 5 tool_poisoning checks, got {len(checks)}"
        for check in checks:
            meta = check.metadata()
    ...
    def test_filter_by_check_ids(self, registry: CheckRegistry) -> None:
        ...
        assert found_ids == {"tp001", "iv001"}, f"Expected tp001 and iv001, got {found_ids}"
    ...
    def test_exclude_ids(self, registry: CheckRegistry) -> None:
        ...
        assert len(excluded) == len(all_checks) - 2, "Excluding 2 checks should reduce total by 2"

    def test_filter_by_multiple_categories(self, registry: CheckRegistry) -> None:
        checks = registry.get_checks(categories=["tool_poisoning", "input_validation"])
        assert len(checks) >= 10, "Should find at least 10 checks across two categories"
    ...
    def test_filter_returns_instances(self, registry: CheckRegistry) -> None:
        ...
        assert isinstance(checks[0], BaseCheck), "Returned check should be a BaseCheck instance"
    ...
    def test_no_matching_filter_returns_empty(self, registry: CheckRegistry) -> None:
        ...
        assert len(checks) == 0, "Non-matching check_ids filter should return empty list"

    def test_no_filters_returns_all(self, registry: CheckRegistry) -> None:
        checks = registry.get_checks()
        assert len(checks) == registry.check_count, "No filters should return all checks"
...
class TestGetCategories:
    ...
    def test_categories_are_sorted(self, registry: CheckRegistry) -> None:
        categories = registry.get_categories()
        assert categories == sorted(categories), "Categories should be returned in sorted order"

    def test_categories_are_unique(self, registry: CheckRegistry) -> None:
        categories = registry.get_categories()
        assert len(categories) == len(set(categories)), "Categories list should have no duplicates"
...
class TestGetSeverityCounts:
    ...
    def test_severity_values_are_valid(self, registry: CheckRegistry) -> None:
        ...
        for severity_key in counts:
            assert severity_key in valid_values, f"Unexpected severity key: {severity_key}"
...
class TestEmptyRegistry:
    ...
    def test_empty_registry_get_all_checks_is_empty(self) -> None:
        reg = CheckRegistry()
        assert reg.get_all_checks() == [], "New registry get_all_checks() should return empty list"

    def test_empty_registry_get_categories_is_empty(self) -> None:
        reg = CheckRegistry()
        assert reg.get_categories() == [], "New registry get_categories() should return empty list"
